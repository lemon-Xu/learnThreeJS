 ...
 * @LastEditTime: 2020-11-08 15:11:16
...
/**
 * @description: 重复纹理 和 回环
 * @param {*}
 * @return {*}
 */
const UseImageMaterialWrap = () => {
    const div = useRef<HTMLDivElement>(null);
    useEffect(() => {
        if (div.current) {
            const width = div.current.clientWidth;
            ...
            const initObject = () => {
                const geometry = new THREE.BoxGeometry(500, 1000, 1000, 4, 4);
                const loader = new THREE.TextureLoader();
                loader.load(
                    '/ks2.jpeg',
                    (texture) => {
                        ...
                        const mesh = new THREE.Mesh(geometry, material)
                        // mesh.position.set(-2000, 500, -500)
                        console.log(texture)
                        scene.add(mesh)
                    },
                    (xhr) => {
...
export { UseBaseImageMaterial, UseImageMaterialWrap };
